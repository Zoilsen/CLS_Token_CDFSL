...
def test_single_ckp(params, remove_featurefile=True):
    ...
    encoder_dict = deepcopy(model.state_dict())
    cnt = 0
    for name in encoder_dict.keys():
        if 'cls_token' in name:
            continue
        #if 'domain_token' in name:
        #    continue
        if name in state_backbone.keys():
            encoder_dict[name] = state_backbone[name]
            cnt += 1
    model.load_state_dict(encoder_dict)
    #print(encoder_dict.keys())
    #print(state_backbone.keys())
    #print(cnt, len(encoder_dict.keys())); exit()
